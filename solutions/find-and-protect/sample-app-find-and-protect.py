...
def configure_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("sample_app_find_and_protect")
...
def protect_file(input_path: Path, output_path: Path, logger: logging.Logger) -> None:
    """Read input file, protect sensitive data, and write to output file."""
    try:
        ...
        with (
            ...
            for line in infile:
                ...
                if stripped_line:  # Only process non-empty lines
                    protected_line = protegrity.find_and_protect(stripped_line) + "\n"
                    outfile.write(protected_line)
                else:
                    outfile.write(line)
        ...
        logger.error("Protection failed: %s", error)
        raise RuntimeError(f"Protection failed: {error}") from error


def log_output_snippet(
    output_path: Path, logger: logging.Logger, snippet_length: int = 250
) -> None:
    """Log a snippet of the protected output file."""
    try:
...
def main() -> None:
    ...
    input_file = base_dir / "shared" / "data" / "input.txt"
    output_file = base_dir / "shared" / "data" / "output-protect.txt"
    config_file = base_dir / "shared" / "config.json"

    config = load_config(config_file)
    if config:
        configure_protegrity(config)

    protect_file(input_file, output_file, logger)
